...
//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				cDescPro := Alltrim(SB1->B1_DESC)+" "+alltrim(SB1->B1_DESCPES)+" "+Rtrim(SB1->B1_ESPECIF)        
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					cDescPro ,; //por solicitaÁ„o Glaucia em 09/06/11 foi troca B1_DESC por B1_VM_P
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
			   ...
			   // Douglas 09/06/11
				_cMesfor := Formula(SF4->F4_FORMULA)		                    
						 
				If !_cMesfor $ cMensFis
					If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
						cMensFis += " "
					EndIf
...
Else                                         
    // cavanna
	cString += '<exporta>'
	cString += '<UFEmbarq>'+ConvType('SP')+'</UFEmbarq>'
	cString += '<locembarq>'+ConvType('BRASIL')+'</locembarq>'
	cString += '</exporta>'	          
EndIf
