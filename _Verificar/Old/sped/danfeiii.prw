...
#DEFINE MAXVALORC 010 //08                                           // Máximo de caracteres por linha de valores numéricos
...
//³Alteração da placa do veiculo retirando do dados adicinal³
//³e reposicionado no campo de placa do veiculo             ³
//³Especifico Cavanna                                       ³
//³Fael                                                     ³
...
//	nPerDesc	:= ((nDesc*100)/nVTotal)
	...
	aAux := {}
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],3))))//AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2])))) alteração fael
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_TOTAL")[1]-5,2))))//AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2])))) alteração do fael
	AADD(aAux, AllTrim(TransForm(nVTotal,TM(nVTotal,TamSX3("D2_TOTAL")[1]-5,2))))//AADD(aAux, AllTrim(TransForm(nVTotal,TM(nVTotal,TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2])))) alteração do fael
	AADD(aAux, AllTrim(TransForm(nDesc,TM(nDesc,TamSX3("D2_DESCON")[1],TamSX3("D2_DESCON")[2]))))
	AADD(aAux, AllTrim(TransForm(nVUniLiq,TM(nVUniLiq,TamSX3("D2_PRCVEN")[1]-6,2)))) //AADD(aAux, AllTrim(TransForm(nVUniLiq,TM(nVUniLiq,TamSX3("D2_PRCVEN")[1],4)))) alateração do fael
	AADD(aAux, AllTrim(TransForm(nVTotLiq,TM(nVTotLiq,TamSX3("D2_TOTAL")[1]-5,2)))) //AADD(aAux, AllTrim(TransForm(nVTotLiq,TM(nVTotLiq,TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2]))))
...
If nfolha>=1
	If lMv_Logod
		oDanfe:SayBitmap(005,075,cLogoD,245,095)
	Else
		oDanfe:SayBitmap(005,075,cLogo,245,095)
	EndIf
Endif
