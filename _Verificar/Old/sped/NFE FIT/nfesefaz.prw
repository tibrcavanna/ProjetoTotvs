...
Local RG      		:= 0  // CAVANNA
Local JP      		:= 0  // CAVANNA
Local aPDCLCV       := {}  // CAVANNA
LocaL xNrNPCL       := {}  // CAVANNA
LOCAL __CPLACA      := ""  // CAVANNA
LOCAL __CNSSPV      := ""  // CAVANNA
LOCAL _cMesEle      := ""  // CAVANNA
...
//                  FIM - CAVANNA
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
						...
						Else
							cMensCli += IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, If( Empty( AllTrim(SF2->F2_MENNOTA) ), AllTrim(SC5->C5_MENNOTA), AllTrim(SF2->F2_MENNOTA) ),AllTrim(SC5->C5_MENNOTA))
						EndIf
					EndIf					
                    // INICIO - CAVANNA
                    IF !EMPTY(SC5->C5_C_PLACA)
						__CPLACA := "Nr. Placa : "+AllTrim(SC5->C5_C_PLACA)
                    ENDIF
				    __CNSSPV := "Nosso Pedido Nr.: "+AllTrim(SC5->C5_NUM)
                    // FIM - CAVANNA
			...
			For RG:= 1 to len(aPDCLCV)
				If aScan(xNrNPCL, aPDCLCV[RG])==0
				    AADD(xNrNPCL, aPDCLCV[RG])
				Endif
			Next

			For JP := 1 to len(xNrNPCL)
			    if JP = 1            
			       if !empty(xNrNPCL[jp])
			          _cMesEle := "* Ped.Cliente: "+alltrim(xNrNPCL[jp])
			       endif   
			    else                     
			       if !empty(xNrNPCL[jp])			    
			          _cMesEle += "/"+alltrim(xNrNPCL[jp])
			       endif   
			    endif
			Next
...
If !Empty(aNota)
	...
	EndIf
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcupc,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,nVicmsDeson)
	cString += "</infNFe>"
EndIf
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,nVicmsDeson)
