...
/*/{Protheus.doc} WSRESTFUL orcamento
orçamentos
...
WSRESTFUL orcamento DESCRIPTION 'orçamentos'
    //Atributos
    WSDATA id         AS STRING
    ...
    //Métodos
    WSMETHOD GET    ID     DESCRIPTION 'Retorna o registro pesquisado' WSSYNTAX '/orcamento/get_id?'                           PATH 'get_id'        PRODUCES APPLICATION_JSON
    WSMETHOD GET    ALL    DESCRIPTION 'Retorna todos os registros'    WSSYNTAX '/orcamento/get_all?{updated_at, limit, page}' PATH 'get_all'       PRODUCES APPLICATION_JSON
END WSRESTFUL
...
WSMETHOD GET ID WSRECEIVE id WSSERVICE orcamento
    Local lRet       := .T.
    ...
    Else
        ...
        Else
            ...
            jResponse['prv1'] := (cAliasWS)->B1_PRV1 
...
WSMETHOD GET ALL WSRECEIVE updated_at, limit, page WSSERVICE orcamento
    Local lRet       := .T.
    Local jResponse  := JsonObject():New()
    Local cQueryTab  := ''
    Local nTamanho   := 10
    ...
    Else
        ...
        While ! QRY_TAB->(EoF())
            ...
            oRegistro['prv1'] := (cAliasWS)->B1_PRV1 
