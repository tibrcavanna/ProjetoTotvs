...
//             U_CAEST002()
//			   	
// Thiago (Rahro) - remoção da chamada U_CAEST002 - conversão para job CAJOB001
User Function SD1100I() //PONTO DE ENTRADA NA INCLUSAO DA NFE POR ITEM
	...
	If SD1->D1_TIPO <> "D"
		...
		If SB1->(dbseek(xfilial("SB1")+SD1->D1_COD))
			...
			//    SB1->B1_ORIGEM  := "2"
			//Endif
			If Empty(SB1->B1_POSIPI) .Or. AllTrim(SB1->B1_POSIPI) <> AllTrim(SD1->D1_POSIPI)
	...
	//##################
	//Desativado em 31/01/2023 devido a construção de atualização por job pela Rahro.
