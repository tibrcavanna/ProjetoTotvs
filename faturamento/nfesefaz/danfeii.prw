...
For nZ := 1 To nLenDet
	...
	If Val(Substr(aAux[1],AT(",",aAux[1])+3,Len(aAux[1]))) <= 0
		aAux[1] := Substr(aAux[1],1,AT(",",aAux[1])+2)
	ElseIf Val(Substr(aAux[1],AT(",",aAux[1])+4,Len(aAux[1]))) <= 0
		aAux[1] := Substr(aAux[1],1,AT(",",aAux[1])+3)
	EndIf

	If Val(Substr(aAux[2],AT(",",aAux[2])+3,Len(aAux[1]))) <= 0
		aAux[2] := Substr(aAux[2],1,AT(",",aAux[2])+2)
	ElseIf Val(Substr(aAux[2],AT(",",aAux[2])+4,Len(aAux[1]))) <= 0
		aAux[2] := Substr(aAux[2],1,AT(",",aAux[2])+3)
	EndIf
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	*/		
	aTamCol := {50,;	// Cod.Prod
				...
				22,;	// CFOP
				14,;	// UN
				iif(aCabec[4] == "CSOSN", 33, 35),; // Quant. - 33,35
				iif(aCabec[4] == "CSOSN", 49, 51),; // V.Unitário 49,53
				42,;	//Vlr. Total - 38
				42,;	//BC. ICMS - 37
				38,;	//V.ICMS - 32
				30,;	//V.IPI - 32
